...
export function createPackageJson(overrides: PackageJsonOverrides = {}): PackageJson {
  ...
  if (dependencyCount !== undefined) {
    ...
    dependencies = depNames.reduce((acc, name) => {
  ...
  if (devDependencyCount !== undefined) {
    ...
    devDependencies = devDepNames.reduce((acc, name) => {
 ...
 */
export function createMonorepoRootPackageJson(
 ...
 */
export function createPackageWithCircularDeps(
  name: string,
  dependsOn: string[]
): PackageJson {
