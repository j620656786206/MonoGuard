...
import { AnalyticsService, EventType, EventProperties } from '../lib/api/services/analytics';
...
export function useAnalytics() {
  ...
  const trackEvent = useCallback(
    (
      ...
      console.log(`Event: ${eventType} - ${eventName}`, { properties, options });
    },
    []
  );
  ...
  const trackAnalysisStart = useCallback(
    (analysisType: string, properties?: EventProperties) => {
  ...
  const trackAnalysisComplete = useCallback(
    (
      ...
      console.log(`Analysis Complete: ${analysisType}`, { duration, healthScore, properties });
    },
    []
  );
  ...
  const trackError = useCallback(
    (errorMessage: string, errorCode?: string, page?: string) => {
  ...
  const trackDownload = useCallback(
    (fileName: string, properties?: EventProperties) => {
  ...
  const trackConversionStep = useCallback(
    (step: string, stepOrder: number, completed: boolean, duration?: number) => {
      console.log(`Conversion Step: ${step}`, { stepOrder, completed, duration });
    },
    []
  );
