...
export class AnalysisService {
  ...
  static async getAnalysisResults(
    id: string
  ): Promise<ApiResponse<AnalysisResult['results']>> {
    return apiClient.get<AnalysisResult['results']>(
      API_ENDPOINTS.ANALYSIS.RESULTS(id)
    );
  }
  ...
  static async getAnalysisDependencies(
    id: string
  ): Promise<ApiResponse<DependencyAnalysis>> {
    return apiClient.get<DependencyAnalysis>(
      API_ENDPOINTS.ANALYSIS.DEPENDENCIES(id)
    );
  }
   ...
   */
  static async getAnalysisArchitecture(
    id: string
  ): Promise<ApiResponse<ArchitectureValidation>> {
    return apiClient.get<ArchitectureValidation>(
      API_ENDPOINTS.ANALYSIS.ARCHITECTURE(id)
    );
  }
  ...
  static async retryAnalysis(id: string): Promise<ApiResponse<AnalysisResult>> {
    return apiClient.post<AnalysisResult>(
      `${API_ENDPOINTS.ANALYSIS.GET(id)}/retry`
    );
  }
  ...
  > {
    return apiClient.get(
      `/analysis/compare?analysis1=${analysisId1}&analysis2=${analysisId2}`
    );
  }
