    ...
    <nav className={cn('flex items-center justify-center', className)}>
      <ol className="flex w-full max-w-4xl items-center">
        {steps.map((step, stepIdx) => (
          <li
            key={step.id}
            className={cn('flex items-center', stepIdx < steps.length - 1 ? 'flex-1' : '')}
          >
            ...
            <div className="flex flex-col items-center">
              <div
                className={cn('flex h-10 w-10 items-center justify-center rounded-full border-2', {
                  ...
                  'border-blue-600 bg-white text-blue-600': step.status === 'current',
                  'border-gray-300 bg-white text-gray-500': step.status === 'pending',
                })}
              >
              ...
              <div className="mt-2 text-center">
                ...
                {step.description && (
                  <div className="mt-1 max-w-24 text-xs text-gray-500">{step.description}</div>
                )}
              </div>
            </div>
