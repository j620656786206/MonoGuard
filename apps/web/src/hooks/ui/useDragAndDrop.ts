...
export interface UseDragAndDropReturn extends DragAndDropState, DragAndDropActions {}
...
export const useDragAndDrop = ({ 
    ...
    return files.filter(file => {
      ...
      return accept.some(acceptType => {
        ...
        if (acceptType.startsWith('.')) {
          return fileName.endsWith(acceptType) || fileName === acceptType.substring(1);
        }
        
        // Check MIME type
        if (acceptType.includes('/')) {
          return fileType === acceptType || fileType.startsWith(acceptType.replace('*', ''));
        }
  ...
  const getFilesFromEvent = useCallback((e: DragEvent<HTMLElement>): File[] => {
    const files: File[] = [];
