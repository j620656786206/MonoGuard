...
- [ ] **Task 1: Clean Up TypeScript CLI** (AC: #1)
  - [ ] 1.1 Backup existing TypeScript CLI structure for reference
  - [ ] 1.2 Remove TypeScript source files:
    ```bash
    rm -rf apps/cli/src
    ...
    ```
  - [ ] 1.3 Keep package.json for npm distribution (will be updated)

- [ ] **Task 2: Initialize Go Module** (AC: #1)
  - [ ] 2.1 Initialize Go module:
    ```bash
    cd apps/cli
    go mod init github.com/j620656786206/MonoGuard/apps/cli
    ```
  - [ ] 2.2 Add Cobra and Viper dependencies:
    ```bash
    go get github.com/spf13/cobra@latest
    go get github.com/spf13/viper@latest
    ```
  - [ ] 2.3 Run `go mod tidy` to clean up dependencies

- [ ] **Task 3: Create Project Structure** (AC: #2)
  - [ ] 3.1 Create directory structure:
    ```bash
    mkdir -p cmd pkg/{config,output}
    ```
  - [ ] 3.2 Create .gitignore for dist/ directory
  - [ ] 3.3 Add placeholder README.md

- [ ] **Task 4: Implement Root Command** (AC: #3, #4)
  - [ ] 4.1 Create `main.go`:
    ```go
    package main
...
- [ ] **Task 5: Implement Placeholder Commands** (AC: #6)
  - [ ] 5.1 Create `cmd/analyze.go`:
    ```go
    package cmd
...
for circular dependencies and other issues.`,
    ...
    ```
  - [ ] 5.4 Create `cmd/init_cmd.go` (avoiding conflict with init function):
    ```go
    package cmd
...
- [ ] **Task 6: Create pkg Utilities** (AC: #4, #6)
  - [ ] 6.1 Create `pkg/config/config.go`:
    ```go
    ...
    ```
  - [ ] 6.2 Create `pkg/output/output.go`:
    ```go
...
- [ ] **Task 8: Update Package Configuration** (AC: #7)
  - [ ] 8.1 Update `apps/cli/package.json` for npm distribution:
    ```json
    {
      "name": "@monoguard/cli",
      "version": "0.1.0",
    ...
    ```
  - [ ] 8.2 Create or update `apps/cli/project.json`:
    ```json
    {
      "name": "@monoguard/cli",
...
- [ ] **Task 9: Verification** (AC: #3, #5, #6, #7)
  - [ ] 9.1 Run `make build` - verify binary builds successfully
  - [ ] 9.2 Run `./dist/monoguard --help` - verify help output
  - [ ] 9.3 Run `./dist/monoguard --version` - verify version info
  - [ ] 9.4 Run `./dist/monoguard analyze` - verify placeholder output
  - [ ] 9.5 Run `./dist/monoguard analyze --format json` - verify JSON output
  - [ ] 9.6 Run `./dist/monoguard check` - verify exit code 0
  - [ ] 9.7 Run `make test` - verify Go tests pass
  - [ ] 9.8 Check binary size: `du -h dist/monoguard`
  - [ ] 9.9 Run `pnpm nx build cli` - verify Nx integration
  - [ ] 9.10 Run `pnpm nx graph` - verify project appears correctly
